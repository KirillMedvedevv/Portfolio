program Main
    ...
    T0 = 200d0 + 273.15d0
    Te = 400d0 + 273.15d0
    dFo = 0.00001d0 !Не забываем про условие устойчисвости
    dX = 0.01d0
    !Вычисляем число итераций внутреннего цикла
    N = dint(1d0/dX)
    if ((N+0d0)*dX > 1d0) N = N - 1 
    p = 7900d0
    c = 500d0
    n = dint(1d0/dX)
    ...
    open(5, file="Ta3(tau).csv")
        ...
        allocate(p_c(N+1), p_f(N+1), data1(N+1), data2(N+1), data3(N+1))
        
        call set_ics(dX, 0d0, 1d0, p_c, N)
        do while(p_c(1) % Fo <= FoMax(1) + dFo)
            call FDM_emp(dFo, dX, p_c, p_f, Bi(1), N) !Расчитали будущий на основе текущего
        ...
        do while(p_c(1) % Fo <= FoMax(2) + dFo)
            CALL FDM_emp(dFo, dX, p_c, p_f, Bi(2), N)
        ...
        do while(p_c(1) % Fo <= FoMax(3) + dFo)
            CALL FDM_emp(dFo, dX, p_c, p_f, Bi(3), N)
            call add_data(data1, data2, data3, p_c, N, Fof(3,:), dFo, T0, Te, d)
            write(5, 20) tautoFo(p_c(1) % Fo, l, d, c, p, -1), ';', TtoTheta(p_c(Xof(1)) % Theta, T0, Te, -1), ';',TtoTheta(p_c(Xof(2)) % Theta, T0, Te, -1), ';',TtoTheta(p_c(Xof(3)) % Theta, T0, Te, -1) 
            p_c = p_f
        end do
        do i = 1, N+1
    ...
    close(2)
    close(3)
    deallocate(p_c, p_f, data1, data2, data3)
    call CPU_TIME(time2)
