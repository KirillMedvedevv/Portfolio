...
implicit none
    ...
    !sin m=5, 14 imp, exp
    call genmesh(2, 2)
    call imp_anti_potok(M, N, dx, dt, mesh)
    ...
    subroutine genmesh(logic, m1)
        ...
        do j = 1, M
            ...
            else if (logic==1) then
                mesh(1, j, 3) = sig2(L, mesh(1, j, 2))
            else if (logic==2) then
                mesh(1, j, 3) = sig3(L, mesh(1, j, 2))
            else
                mesh(1, j, 3) = sig4(L, mesh(1, j, 2))
            end if
        end do
    end subroutine
    ...
    function sig3(L, x) result(y)
        real(8) :: L, x, y
        if (x >= 0d0 .and. x<=0.5d0*L) then
            y = 1d0
        else if (x > 0.5d0*L .and. x<=L) then
            y = 2d0
        end if
    end function
    
    function sig4(L, x) result(y)
        real(8) :: L, x, y
        if (x >= 0d0 .and. x<=0.5d0*L) then
            y = 2d0
        else if (x > 0.5d0*L .and. x<=L) then
